...
const DatePicker: React.FC<DatePickerProps> = ({
	...
	let [date, setDate] = useState<any>(),
		[temporaryDate, setTemporaryDate] = useState<any>(),
		[stringDate, setStringDate] = useState<string>(''),
		[isVisible, setIsVisible] = useState<boolean>(false),
		[isCalendarReady, setIsCalendarReady] = useState<boolean>(false),
		datePickerRef = useRef<any>(),
		inputRef = useRef<any>(),
		calendarRef = useRef<any>(),
		ref = useRef<any>({}),
	...
	useEffect(() => {
		function handleClickOutside(event: any) {
			...
			[inputRef.current, calendarRef.current].forEach((element: any) => {
				if (
					element &&
	...
	useEffect(() => {
		...
		if (range || isArray(date)) {
			...
			date = date.map(checkDate).filter((value: unknown) => value !== undefined);
	...
	const mustPickNewDate = () => {
		return onOpenPickNewDate && !value && !ref.current.date && !range;
	};

	const handleChange = (date: any, force?: boolean) => {
	...
	const handleFocusedDate = (focusedDate: any, clickedDate: any) => {
	...
	const getStringDate = (date: any, separator?: string) => {
