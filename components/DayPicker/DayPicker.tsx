...
// interface
import { DayPickerProps, StateType } from './DayPickerInterface';
...
const DayPicker: React.FC<DayPickerProps> = ({
	...
	const getMonths = (date: any, numberOfMonths: number, weekStartDayIndex: number) => {
		if (!date) return [];
	...
	const getClassName = (object: any, numberOfMonths: number) => {
		let names = [
				// "rmdp-day",
	...
	function isSelected(dateObject: any) {
		return [].concat(selectedDate).some((date) => isSameDate(date, dateObject));
	}

	function getAllProps(object: any) {
	...
	function mustDisplayDay(object: any) {
		if (object.current) return true;
	}

	function selectDay(
		{ date: dateObject, current }: StateType | any,
		monthIndex: number,
	...
	return (
		...
		<div className={`w my-7 flex gap-8`} onMouseLeave={() => rangeHover && setDateHovered()}>
